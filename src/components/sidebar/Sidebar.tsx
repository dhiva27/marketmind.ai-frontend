...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed lg:static inset-y-0 left-0 z-50 w-64 lg:w-[260px] bg-[#0F0F12] text-slate-100 p-4 flex flex-col justify-between select-none transition-all duration-200 ease-out border-r border-slate-900',
      ...
      >
        <div className="flex flex-col h-full min-h-0 space-y-4">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-bold text-sm flex items-center justify-center gap-2 shadow-md transition-all duration-200 hover:scale-[1.01] active:scale-[0.99]"
          >
            <Plus className="w-4 h-4 stroke-[2.5]" />
            <span>New Chat</span>
          </button>

          {/* Navigation Links - Medium font size */}
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:text-white hover:bg-slate-900/90 transition-all duration-200"
                >
                  <Icon className="w-4.5 h-4.5 text-slate-400 shrink-0 stroke-[1.75]" />
                  <span>{item.name}</span>
                </Link>
              );
            })}
              ...
              {recentChatsList.map((chat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[11px] text-slate-500 shrink-0 ml-2">{chat.time}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="pt-3 border-t border-slate-900/80 relative" ref={menuRef}>
            {/* User Profile Trigger Button */}
            <button
              onClick={() => setProfileMenuOpen(!profileMenuOpen)}
              className="w-full flex items-center justify-between p-2 rounded-xl hover:bg-slate-900/90 transition-all duration-200 group text-left"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="relative w-8.5 h-8.5 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 flex items-center justify-center text-white text-xs sm:text-sm font-bold shrink-0 overflow-hidden shadow-xs">
                  ...
                  <div className="absolute bottom-0 right-0 w-2 h-2 rounded-full bg-emerald-500 border border-slate-950" />
                </div>
                <div className="min-w-0">
                  <div className="font-bold text-xs sm:text-sm text-white truncate">{displayName}</div>
